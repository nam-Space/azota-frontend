...
const RightManageExercise = () => {
    ...
    useEffect(() => {
        if (router.query.exerciseId && router.query.classroomId) {
            ...
            dispatch(fetchUser({ query: `page=1&limit=500&filter.userClassrooms.classroom.classroomExercises.exerciseId=$eq:${exerciseId}&filter.userClassrooms.classroom.classroomExercises.classroomId=$eq:${classroomId}&relations=userClassrooms.classroom.classroomExercises,histories&sort=updatedAt-DESC` }))
    ...
    const columns: ProColumns<IUser>[] = [
        {
            title: <FormattedMessage id="classroomDetail-exerciseDetail.right-container.table.col.no" />,
            dataIndex: 'stt',
        ...
        {
            title: <FormattedMessage id="classroomDetail-exerciseDetail.right-container.table.col.name" />,
            dataIndex: 'name',
            sorter: true,
            render: (text, record, index, action) => {
                return (
        ...
        {
            title: <FormattedMessage id="classroomDetail-exerciseDetail.right-container.table.col.max-score" />,
            dataIndex: 'score',
            render: (text, record, index, action) => {
                ...
                return (
                    <span style={{ fontWeight: 600, color: ((!datetimeScoreMaxMilliseconds && new Date().getTime() <= timeEndMilliseconds) || (datetimeScoreMaxMilliseconds && datetimeScoreMaxMilliseconds <= timeEndMilliseconds && scoreMax >= 5)) ? 'black' : '#FF0000' }}>
                        {datetimeScoreMaxMilliseconds && datetimeScoreMaxMilliseconds <= timeEndMilliseconds
                            ? scoreMax
                            : (!datetimeScoreMaxMilliseconds && new Date().getTime() <= timeEndMilliseconds) ? <FormattedMessage id='classroomDetail-exerciseDetail.right-container.table.row.status.waiting' /> : <FormattedMessage id='classroomDetail-exerciseDetail.right-container.table.row.status.unsubmitted' />}
        ...
        {
            title: <FormattedMessage id="classroomDetail-exerciseDetail.right-container.table.col.created-at" />,
            hideInSearch: true,
            render: (text, record, index, action) => {
                const scoreMax = Math.max(...(record.histories?.filter(history => history.classroomExerciseId === classroomExercise.id).map(history => history.score) as number[]))
                const datetimeScoreMax = (record.histories?.find(history => history.classroomExerciseId === classroomExercise.id && history.score == scoreMax)?.createdAt)
        ...
        {
            title: <FormattedMessage id="classroomDetail-exerciseDetail.right-container.table.col.duration" />,
            hideInSearch: true,
            render: (text, record, index, action) => {
                const scoreMax = Math.max(...(record.histories?.filter(history => history.classroomExerciseId === classroomExercise.id).map(history => history.score) as number[]))
                const datetimeScoreMax = (record.histories?.find(history => history.classroomExerciseId === classroomExercise.id && history.score == scoreMax)?.createdAt)
        ...
        {
            title: <FormattedMessage id="classroomDetail-exerciseDetail.right-container.table.col.action" />,
            hideInSearch: true,
            render: (text, record, index, action) => {
                const scoreMax = Math.max(...(record.histories?.filter(history => history.classroomExerciseId === classroomExercise.id).map(history => history.score) as number[]))
                const datetimeScoreMax = (record.histories?.find(history => history.classroomExerciseId === classroomExercise.id && history.score == scoreMax)?.createdAt)
                    ...
                    <span>
                        <Button style={{ background: theme }} type='primary' icon={<ClockCircleOutlined />}>
                            <Link href={`/student/${record.id}/classroomExercise/${classroomExercise.id}`}>
                                <FormattedMessage id='classroomDetail-exerciseDetail.right-container.table.row.action.view-history' />
                            </Link>
                        </Button>
                    </span>
                )
    ...
    const buildQuery = (params: any, sort: any, filter: any) => {
        ...
        if (router.query.exerciseId && router.query.classroomId) {
            const exerciseId = router.query.exerciseId
            const classroomId = router.query.classroomId
            temp += `&filter.userClassrooms.classroom.classroomExercises.exerciseId=$eq:${exerciseId}&filter.userClassrooms.classroom.classroomExercises.classroomId=$eq:${classroomId}`
        }
    ...
    return (
        <div className={styles['wrapper']}>
            <h2 className={styles['heading-title']}><FormattedMessage id="classroomDetail-exerciseDetail.right-container.classroom" /> {classroomExercise.classroom?.name}</h2>

            <div className={styles['table-user']}>
                <DataTable<IUser>
                    actionRef={tableRef}
                    headerTitle={<FormattedMessage id="classroomDetail-exerciseDetail.right-container.table.title" />}
                    rowKey="id"
                    loading={isFetching}
                    columns={columns}
                    dataSource={users}
...
export default RightManageExercise